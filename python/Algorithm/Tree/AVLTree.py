...
class AVLTree:
    ...
    def __deleteNode(self, tNode:AVLNode) -> AVLNode:
        if tNode.left == self.NIL and tNode.right == self.NIL: # 자식이 없는 케이스
            return self.NIL
    ...
    def __rightRotate(self, t:AVLNode) -> AVLNode:
        ...
        t.left = LRChild
        t.height = 1 + max(t.left.height, t.right.height)
        LChild.hegiht = 1 + max(LChild.left.height, LChild.right.height)
        return LChild
    ...
    def __needBalance(self, t:AVLNode)-> int:
        ...
        if(t.left.height + 2 <= t.right.height):
            ...
            if t.left.left.hegiht >= t.left.right.height:
                type = self.LL
            else:
                type = self.LR
        else:
